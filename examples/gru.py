...
data_batch_args = {}
data_init_kwargs = {'n_tap': 40}#'shift_augment': False}
optimiser_params = dict(lr=1.4e-3, mom=0.9, wd=0.5e-3)
batch_size = 40
...
def create_model():
    ...
    inp_mem = neuromancer.InitialState_like(inp0, override_f=750, init_kwargs=dict(mode='fix-uni', scale=0.1))
    out = neuromancer.GRU(inp0, inp_mem, 750)
    out = neuromancer.Scan(out, inp_mem, in_iterate=inp, in_iterate_0=inp0, n_steps=40,
                     last_only=True)
    ...
    loss  = neuromancer.MultinoulliNLL(out, target, name='nll', target_is_sparse=True,
