...
save_path = '~/CNN_Training/3d_NUMA/'
save_name = '3d_cnn'

preview_data_path = '/docs/devel/data/neuro_data/preview_cubes.h5'
...
data_init_kwargs = dict(d_path='/docs/devel/data/neuro_data/',
                        l_path='/docs/devel/data/neuro_data/',
                        d_files=[('raw_%i.h5' %i, 'raw') for i in range(3)],
                        l_files=[('barrier_int16_%i.h5' %i, 'lab') for i in range(3)],
                        aniso_factor=2, valid_cubes=[2,])

data_batch_args = dict(grey_augment_channels=[0], warp=0.5, warp_args=dict(
                       sample_aniso=True, perspective=True))
...
optimiser = 'Adam'
optimiser_params = dict(lr=0.0005, mom=0.9, beta2=0.999, wd=0.5e-4)
