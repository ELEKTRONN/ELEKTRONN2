...
__all__ = ['scroll_plot', 'embedfilters', 'sma',
           'plot_debug', 'plot_hist', 'plot_var', 'my_quiver',
           'plot_trainingtarget', 'plot_kde']
...
class Scroller(object):
    def __init__(self, axes, images, names, init_z=None):
        ...
        for ax, dat, name in zip(axes, images, names):
            if name in ['id', 'ids', 'ID', 'IDs', 'seg', 'SEG','Seg',
                        'lab', 'label', 'Label']:
                cmap = 'nipy_spectral'
            else:
                cmap = 'gray'

            self.imgs.append(ax.imshow(dat[self.ind], interpolation='None', cmap=cmap))
            ax.set_xlabel(name)
...
def scroll_plot(images, names=None, init_z=None):
    ...
    >>> fig, scroller = scroll_plot4(images, names)
    >>> fig.show()
    ...
    if isinstance(images, np.ndarray):
        return scroll_plot1(images, names, init_z)
    elif len(images)==2:
        assert len(names)>=2
        return scroll_plot2(images, names, init_z)
    elif len(images)==4:
        assert len(names)>=4
        return scroll_plot4(images, names, init_z)

def scroll_plot1(image, name, init_z):
    """
...
def scroll_plot2(images, names, init_z):
    """
    Creates a plot 1x2 image plot of 3d volume images
    Scrolling changes the displayed slices
...
def scroll_plot4(images, names, init_z):
    """
...
def embed3d2d(a, border_width=1, normalize=False, output_ratio=1.5,):
    """
    ...
    ret = np.zeros((nvert*(border_width+sh[1]), nhor*(border_width+sh[2])),
                   dtype=np.float32)
...
def embedfilters(filters, border_width=1, normalize=False,
                 output_ratio=1.0, rgb_axis=None):
    """
    ...
    if rgb_axis is not None:
        ...
        for i in range(3):
            ...
            slice[rgb_axis] = i
            f = filters[slice]
            channels.append(embedfilters(f, border_width, normalize, output_ratio))
        return np.dstack(channels)

    if filters.ndim==3:
        return embed3d2d(filters, border_width, normalize, output_ratio)
    elif filters.ndim>3:
        parts = []
        for f in filters:
            parts.append(embedfilters(f, border_width, normalize, output_ratio))
...
def plot_hist(timeline, history, save_name,
        ...
        try:
            ...
            plt.subplot(211)
            plt.plot(x_timeline, timeline['loss'], 'b-', alpha=0.5, label='Update Loss'  )
            ...
            plt.hlines(lt_m+lt_s, 0, N, linestyle='dotted', colors='r', linewidth=1)
            plt.hlines(lt_m-lt_s, 0, N, linestyle='dotted', colors='r', linewidth=1)
            plt.xlabel('Update steps %s, total runtime %s'%(N-1, runtime))

            ax = plt.twiny()
            if timeline['time'][-1] > 120*60:
                add_timeticks(ax, timeline['time'] / 3600, x_timeline, time_str='hours')
            else:
                add_timeticks(ax, timeline['time']/60, x_timeline, time_str='mins')

            ### Loss vs Prevalence ###
            plt.subplot(212)
            c = 1.0 - (timeline['time']/timeline['time'].max())
            plt.scatter(timeline['batch_char'],timeline['loss'],
                        c=c, marker='.', s=80, cmap='gray', edgecolors='face')
            if autoscale:
            ...
            plt.plot(history['steps'], history['lr'], 'r-', label='LR', linewidth=3)
            #plt.xlabel('Update steps %s, total runtime %s'%(N-1, runtime))
            plt.legend(loc=3)
            ...
            if not np.any(np.isnan(sma(history['valid_err'],8))):
                plt.plot(history['steps'], sma(history['valid_err'],8),
                         'r-', label='Smooth valid Error', linewidth=3)
            if autoscale:
...
def plot_var(var, save_name):
    ...
    plt.subplot(221)
    plt.scatter(var[:,1], var[:,3], c=c, marker='.', s=80,
                cmap='gray', edgecolors='face')
    plt.title("Concentration vs. NLL")

    plt.subplot(222)
    plt.scatter(var[:,2], var[:,3], c=c, marker='.', s=80,
                cmap='gray', edgecolors='face')
    plt.title("Concentration vs. NLL.std")

    plt.subplot(223)
    plt.scatter(var[:,3], var[:,4], c=c, marker='.', s=80,
                cmap='gray', edgecolors='face')
    plt.title("Concentration vs. Concentration.std")

    plt.subplot(224)
    plt.scatter(var[:,1], var[:,2], c=c, marker='.', s=80,
                cmap='gray', edgecolors='face')
...
def plot_regression(pred, target, save_name,
              loss_smoothing_length=200, autoscale=True):
        """Plot graphical info during Training"""
        try:
            # Subsample points for plotting
            ...
            ### Loss vs Prevalence ###
            plt.scatter(pred, target, c=c, marker='.', s=80, cmap='gray',edgecolors='face')
...
def my_quiver(x, y, img=None, c=None):
    ...
    plt.quiver(x, y, c, angles='xy', units='xy', cmap='spring', pivot='middle', scale=0.5)
    return figure
...
def plot_trainingtarget(img, lab, stride=1):
    ...
    if len(lab) * stride != len(img):
        ...
        if lab.ndim==3:
            ...
            new_t = np.zeros((lab.shape[0] + 2 * off, lab.shape[1] + 2 * off, 3))
...
def plot_exectimes(exectimes, save_path='~/exectimes.png', max_items=32):
    ...
    :param exectimes: OrderedDict of execution times (output of Model.measure_exectimes())
    :param save_path: Where to save the plot
    :param max_items: Only the max_items largest execution times are given names and are plotted independently.
