...
class DefaultConfig(object):
    def __init__(self):
        ...
        """
        ### Toolkit Setup ###------------------------------------------------------
        # (*) <String>: where to create the CNN directory. In this directory a new folder is created with the name of the model
        ...
        self.background_processes = False            # <Bool>/<Int>: whether to "pre-fetch" batches in separate background process, <Bool> or number of processes (True-->2)
        ...
        self.inspection = False # Flag used to conditionally do something different for inspection/debugging
        self.backupsrc = True  # Whether to create a backup of the current source code in the save directory
        ...
        # lfile_path = os.path.join(self.save_path, 'elektronn2.log')
        # lfile_level = logging.DEBUG
        # lfile_handler = logging.FileHandler(lfile_path)
        # lfile_handler.setLevel(lfile_level)
        # lfile_handler.setFormatter(lfile_formatter)
    ...
    def read_user_config(self):
        ...
        if not os.path.exists(user_path):
            ...
            for key in config_dict:
                ...
                if key=='param_save_h':
                    raise DeprecationWarning("Don't use param_save_h, use param_save_it")
