...
class SharedMem(object):
    ...
    _numpy_to_ctypes = dict(list(zip(_ctypes_to_numpy.values(), _ctypes_to_numpy.keys())))
    ...
    def puthandle(self, dtype, shape, data=None, lock=False):
        ...
        if self.profile:
            t_alloc = t1 - t0
            t_write = t2 - t1
            self.logger.info('SharedMemAlloc %g ms, WriteInitialData %g ms' %(t_alloc*1000, t_write*1000))
...
class Proc(mp.Process):
    ...
    """
    def __init__(self, mp_arrays, shapes, events, target, target_args, target_kwargs, profile):
    ...
    def run(self):
        while True:
            try:
                self.events['new'].wait() # wait till host has fetched data from shm and demands new data from this proc
...
class BackgroundProc(SharedMem):
    def __init__(self, target, dtypes=None, shapes=None, n_proc=1, target_args=(), target_kwargs={}, profile=False):
        ...
        for k in range(n_proc): # create a list of mp-arrays for each process
            a = []
    ...
    def get(self, timeout=False):
        ...
        if self.profile:
            t_wait = t1 - t0
            t_write = t2 - t1
            self.logger.info('Waiting for subprocess %g ms, converting to numpy %g ms' %(t_wait*1000, t_write*1000))
...
class SharedQ(SharedMem):
    ...
    def startproc(self, dtypes, shapes, target, target_args=(), target_kwargs={}):
        """
    ...
    def get(self):
        ...
        if missing > 0:
            self.logger.warning("You should have started %i new workes before Q.get()" % missing)
