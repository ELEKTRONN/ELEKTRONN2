        ...
        try:
            try:
                inp = prompt_toolkit.prompt(
                    u"%s@neuromancer: " % user_name,  # needs to be an explicit ustring for py2-compat
                    history=ptk_hist,
            ...
            except Exception:  # Catch all exceptions in order to prevent catastrophes in case ptk suddenly breaks
...
class Schedule(object):
    ...
    def update(self, iteration):
        if self.mode=='mult':
            self.variable_setter(np.multiply(self.variable_getter(), self.factor))
            self.next_update = iteration + 1000

        elif self.mode=='set':
            if iteration==self.update_steps[0]: # process value from schedule list
                ...
                if len(self.update_steps)==0: # schedule finished
                    ...
                    self.next_update = min(iteration + 1000, self.update_steps[0])
            ...
            if iteration<self.n_steps:
                self.variable_setter(np.subtract(self.variable_getter(), self.delta))
                self.next_update = iteration + 1000
    ...
    def bind_variable(self, variable_param=None, obj=None, prop_name=None):
        ...
        if variable_param is not None and \
                hasattr(variable_param, 'get_value') and \
                hasattr(variable_param, 'set_value'):
        ...
        # This is (most likely a theano.SharedVariable) wrapped as property of model-obj
            ...
            if hasattr(obj.__class__, prop_name): # it is a property, get the s/getters
            ...
            if hasattr(obj, prop_name): # it is a atrribute, create the s/getters
                ...
                if hasattr(obj, 'nontrainable_params'):
                    ...
                    try:
                        ...
                        self.variable_setter = variable_param.set_value
                        self._target = variable_param.name
                        logger.debug("Found %s in model.nontrainable_params" %variable_param.name)
                    except:
                        raise AttributeError("%s not found or is not VariableParam!" % prop_name)
                else:
                    raise AttributeError("%s is not a property/attribute of %s "
                                         "and not in model.nontrainable_params!" %(prop_name, obj.__class__))
...
class HistoryTracker(object):
    def __init__(self):
        ...
        self.history = utils.AccumulationArray(n_init=int(1e4), dtype=dict(
            names=[u'steps', u'time',u'train_loss', u'valid_loss', u'loss_gain',
                   u'train_err', u'valid_err', u'lr', u'mom',u'gradnetrate'],
            formats=[u'i4',]+[u'f4',]*9))
    ...
    def save(self, save_name):
        ...
        utils.picklesave([self.timeline, self.history, self.debug_outputs,
                          self.debug_output_names, self.regression_track], file_name)

    def load(self, file_name):
        self.timeline, self.history, self.debug_outputs,\
        self.debug_output_names, self.regression_track = utils.pickleload(file_name)
...
#        self.plotting_proc.start()
        ...
        if self.debug_output_names and self.debug_outputs.length:
            plotting.plot_debug(self.debug_outputs, self.debug_output_names, save_name)

        if self.regression_track:
            plotting.plot_regression(self.regression_track[0],
                                     self.regression_track[1],
                                     save_name)
...
class ExperimentConfig(object):
    ...
    def __init__(self, exp_file, host_script_file=None, use_existing_dir=False):
        ...
        old_lfile_handler = [x for x in logger.handlers if isinstance(x, logging.FileHandler)][0]
        ...
        lfile_path = os.path.join(self.save_path, '%s.log' %('0-'+self.save_name,))
        ...
        change_logging_file(inspection_logger, self.save_path, file_name='%s.inspection-log' %('0-'+self.save_name,))
        # old_lfile_handler = [x for x in inspection_logger.handlers if isinstance(x, logging.FileHandler)][0]
        #
        # inspection_logger.removeHandler(old_lfile_handler)
        # lfile_formatter = logging.Formatter('%(message)s', datefmt='%H:%M')
        # lfile_path = os.path.join(self.save_path, '%s.inspection-log' %('0-'+self.save_name,))
        # lfile_level = logging.DEBUG
        # lfile_handler = logging.FileHandler(lfile_path)
        # lfile_handler.setLevel(lfile_level)
        # lfile_handler.setFormatter(lfile_formatter)
    ...
    def read_user_config(self):
        ...
        custom_dict = dict()
        exec (compile(open(self.exp_file).read(), self.exp_file, 'exec'), {}, custom_dict)
        ...
        for key in custom_dict:
            if key in allowed:
                if key in['save_path', 'model_load_path',
                          'preview_data_path', 'data_class']:
                    try:
                        if key=='data_class' and isinstance(custom_dict[key], tuple):
                            custom_dict[key][0] = os.path.expanduser(custom_dict[key][0])
                        else:
                            custom_dict[key] = os.path.expanduser(custom_dict[key])
                    except:
                        pass
                ...
                if getattr(config, key)!=custom_dict[key]:
                    logger.info('Overriding default config "%s" (%s) with value: %s'
        ...
        if didyoumean:
            ...
            for key, suggestion in didyoumean:
                logger.warning('"{}":\tbest match: "{}"?'.format(key, suggestion))
        ...
        if self.lr_schedule:
            raise ValueError()
            self.schedules['lr'] = self.lr_schedule
        if self.mom_schedule:
            raise ValueError()
            self.schedules['mom'] = self.mom_schedule
        if self.wd_schedule:
            raise ValueError()
            self.schedules['wd'] = self.wd_schedule
        if self.dropout_schedule:
            raise ValueError()
            self.schedules['dropout_rates'] = self.dropout_schedule
        if self.gradnet_schedule:
            raise ValueError()
            self.schedules['gradnet_rates'] = self.gradnet_schedule
    ...
    def check_config(self):
        for k,v in self.__dict__.items():
            if k=='create_model' and v is None:
                ...
                if k not in ['class_weights', 'lazy_labels', 'gradnet_schedule',
                             'dropout_schedule', 'lr_schedule', 'wd_schedule',
                             'mom_schedule', 'schedules',
                             'preview_kwargs', 'preview_data_path',
                             'model_load_args', 'sequence_training']:
    ...
    def make_dir(self):
        ...
        if config.backupsrc:
            # Save the full source code that was used to train the network (takes about 180ms).
            try:
                ...
                logger.info('Archived source code in {}.tar.gz'.format(backuppath))
            except Exception:
                logger.warning('Failed to archive the package source code in {}/Backup/'.format(self.save_path))
...
def eval_thresh(args):
    ...
    perf = performance_measure(tp, tn, fp, fn) # tpr, fpr, precision, recall, bal_accur, accur, f1
...
def evaluate(gt, preds, save_name, thresh=None, n_proc=None):
  ...
  thresh = find_nearest(threshs, thresh) if thresh else threshs[perf[5,:].argmax()]
...
def evaluate_model_binary(model, name, data=None, valid_d=None, valid_l=None,
                          train_d=None, train_l=None, n_proc=2, betaloss=False, fudgeysoft=False):
    ...
    train_gt = [gt>0.5 for gt in train_gt]  # binarise possibly probabilistic GT

    train_acc, train_area, train_thresh = evaluate(train_gt,
                                  train_preds,
                                  "1-"+name+"_train")
    ...
    valid_gt = [gt>0.5 for gt in valid_gt]  # binarise possibly probabilistic GT

    valid_acc, valid_area, valid_thresh = evaluate(valid_gt, valid_preds, "1-" + name + "_valid")
    ...
    report_str += "%.3f,\t%.3f,\t%.3f,\t%.3f,\t%.3f,\t" %(valid_nll, valid_acc,
                                                          valid_area,
                                                          train_acc-valid_acc,
    ...
    for i,(l, p) in enumerate(zip(valid_gt, valid_preds)):
        ...
        ri, best_ri, seg = image.optimise_segmentation(l, p_int, "2-"+name+"_valid_%i"%i, n_proc=n_proc)
