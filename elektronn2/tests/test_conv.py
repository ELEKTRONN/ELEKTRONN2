...
def test_conv():
    if False:
        ...
        W_val = np.random.rand(*fil_shape).astype(np.float32)  # (nof, z, ch, xf, yf)
        ...
        f1 = elektronn2.neuromancer.graphutils.make_func([x], y1, profile_execution=10)
    ...
    if True:
        ...
        fil_shape  = (7, 5, 3,  3,  3)
        x_val = np.random.rand(*sig_shape).astype(np.float32)
        W_val = np.random.rand(*fil_shape).astype(np.float32)  # (nof, ch, zf xf, yf)
        ...
        y2 = conv3d(x.dimshuffle(0,2,1,3,4), W.dimshuffle(0,2,1,3,4)).dimshuffle(0,2,1,3,4)

        f1 = elektronn2.neuromancer.graphutils.make_func([x], y1, profile_execution=5)
        f2 = elektronn2.neuromancer.graphutils.make_func([x], y2, profile_execution=5)

        r3 = np.array(f1(x_val))
        r4 = f2(x_val)
        assert np.allclose(r3, r4) # cudnn and reshaped conv2d3d give same result, but cudnn ist faster!

        y1 = dnn.dnn_conv3d(x, W, border_mode='valid')
        y1 = dnn.dnn_pool(y1, (2,2,2), stride=(2, 2, 2), pad=(0,0,0), mode='max')
        f1 = elektronn2.neuromancer.graphutils.make_func([x], y1, profile_execution=5)
        r3 = np.array(f1(x_val))
        ...
        f2 = elektronn2.neuromancer.graphutils.make_func([x], y2, profile_execution=5)
        r4 = f2(x_val)
        assert np.allclose(r3, r4.transpose(0,2,1,3,4)) # pooling als works, not it is not so much faster anymore....
        ...
        f2 = elektronn2.neuromancer.graphutils.make_func([x], y2, profile_execution=5)
        r4 = f2(x_val)
        assert np.allclose(r3, r4.transpose(0,2,1,3,4), atol=1e-5) # sm also works but diff is ~1e-5
