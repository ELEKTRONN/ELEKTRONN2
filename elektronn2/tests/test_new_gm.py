...
def test_new_gm():
    ...
    out   = neuromancer.Conv(inp, 20,  (1,6,6), (1,1,1), mfp=mfp, batch_normalisation='train')
    ...
    out   = neuromancer.Conv(out, 80,  (4,1,1), (2,1,1), mfp=mfp)# first z kernel, 2 pool
    ...
    probs   = neuromancer.Conv(out1, 2, (1,1,1), (1,1,1), mfp=mfp, activation_func='lin')
    probs   = neuromancer.Softmax(probs, name='probs')
    discard, mode = neuromancer.split(probs, 1, n_out=2)

    concentration = neuromancer.Conv(out2, 1, (1,1,1), (1,1,1),
                               mfp=mfp, activation_func='lin', name='concentration')
    ...
    prediction = neuromancer.Concat([mode, concentration], axis=1, name='prediction')
    ...
    probs2   = neuromancer.Conv(out2, 2, (1,1,1), (1,1,1), mfp=mfp, activation_func='lin')
