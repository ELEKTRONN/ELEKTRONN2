...
def test_cnndata():
    ...
    data_init_kwargs = dict(d_path='~/lustre/mkilling/BirdGT/j0126_new/',
                            ...
                            d_files=[('raw_%i.h5' % ii, 'raw') for ii in range(3)],
                            l_files=[('barrier_int16_%i.h5' % ii, 'lab') for ii in range(3)],
                            n_target=2,
    ...
    kwargs = dict(batch_size=1, flip=True,
    ...
    if False: # BG
        bg = BackgroundProc(D.getbatch, n_proc=2, target_args=(), target_kwargs=kwargs, profile=False)
        try:
            for i in range(1):
                ...
                aff_pred = label*0.95 + 0.05*np.random.uniform(size=label.shape)
                pos_counts, neg_counts  = malis.get_malis_weights(aff_pred, label, seg, nhood)
                ...
                plt.ioff()
                fig, scroller = scroll_plot.scroll_plot([aff_pred, seg_color, seg_color_gt, neg_counts],
                                                           ['PRED', 'SEG', 'SEG_GT', 'NEG'])
                fig.show()
                plt.show()
    ...
    if True:
        ...
        for z in range(lab.shape[0]):
            plt.imsave('/tmp/%i-lab.png'%(z+off), lab[z].astype(np.uint8)*255)
...
if __name__ == "__main__":
    ...
    from elektronn2.utils.plotting import scroll_plot2, scroll_plot
...
#    data.train_l[0][:,::10,:,:] *= 0.5
    ...
    sample_warp_params = dict(sample_aniso=True, lock_z=True,
                                  no_x_flip=False, warp_amount=1, perspective=True)
    ...
    fig = scroll_plot2([img, (0.4*target+0.6*img)], ['raw', 'target'])
