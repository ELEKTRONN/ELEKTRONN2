...
def test_schedule_config():
    ...
    data_init_kwargs = dict(d_path='~/lustre/mkilling/BirdGT/j0126_new/',
                            ...
                            d_files=[('raw_%i.h5' %i, 'raw') for i in range(1)],
                            l_files=[('barrier_int16_%i.h5' %i, 'lab') for i in range(1)],
                            valid_cubes=[],
                            n_target=2,
    ...
    data_batch_args = dict(flip=True,
    ...
    mom_schedule = dict(updates=[(500,0.8), (1000,0.7), (1500,0.9), (2000, 0.2)])
