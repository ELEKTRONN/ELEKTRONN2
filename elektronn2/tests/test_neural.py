...
def test_neural():
    ...
    if False: # Perce[tron]
        ...
        y = Perceptron(x, 5, activation_func='prelu', W=W, b=b, batch_normalisation='train')
        ...
        b = np.arange(2*5, dtype=np.float32).reshape(5,2)
        y = Perceptron(x, 5, activation_func='prelu', W=W, b=b, batch_normalisation='train', flatten=True)
        print(y.all_params_count)
        ret = y(x_val)
    ...
    if True: # visual 2d conv + MFP
        img = plt.imread('/docs/devel/svn/Marius/Lichtenstein.png').transpose((2,0,1))[None,]
...
#        y4 = Conv(y3, 64, (4,4,4), (1,1,1), mfp=False, name='Fragments')
        ...
        y4 = Conv(y3, 64, (4,4,4)[::-1], (1,1,1)[::-1], mfp=False, name='Fragments')
    ...
    data0, _ = neuromancer.split(data, axis='r', index=1, strip_singleton_dims=True)
    mem_init0 = neuromancer.InitialState_like(data0, override_f=13, init_kwargs=dict(mode='fix-uni', scale=0.1))
    mem_init1 = neuromancer.InitialState_like(data0, override_f=26, init_kwargs=dict(mode='fix-uni', scale=0.1))
    ...
    gru_s  = neuromancer.Scan(gru_state, mem_init0, gru_state, in_iterate=data, in_iterate_0=data0)
    lstm_s = neuromancer.Scan(lstm_state, mem_init1, lstm_state, in_iterate=data, in_iterate_0=data0)
